...
public class MigrationManager extends SimpleManager<CoinsEnginePlugin> {
    ...
    public boolean startMigration(@NotNull CommandSender sender, @NotNull String name, @NotNull Currency currency) {
        ...
        this.plugin.runTaskAsync(task -> {
            ...
            this.plugin.runTask(() -> Lang.MIGRATION_STARTED.message().send(sender, replacer -> replacer.replace(Placeholders.GENERIC_NAME, migrator.getName())));
            this.migrate(migrator, currency);
            this.plugin.runTask(() -> Lang.MIGRATION_COMPLETED.message().send(sender, replacer -> replacer.replace(Placeholders.GENERIC_NAME, migrator.getName())));
